...
void DRV_CAN0_Initialize(void)
{
    ...
    PLIB_CAN_BaudRatePrescaleSet(CAN_ID_2, 9); // set to 1 higher then ECAN tool
    ...
    /* Configure CAN_ID_2 Channel for CAN_TX_RTR_ENABLED operation. Allocate 1 message buffer, and assign low medium priority for transmissions. */
    PLIB_CAN_ChannelForTransmitSet(CAN_ID_2, CAN_CHANNEL0, 1, CAN_TX_RTR_ENABLED, CAN_LOW_MEDIUM_PRIORITY);
    /* Configure CAN_ID_2 Channel for CAN_RX_FULL_RECEIVE operation. Allocate 1 message buffer, and assign low medium priority for transmissions. */
    ...
    PLIB_CAN_ChannelEventEnable(CAN_ID_2, CAN_CHANNEL1, CAN_RX_CHANNEL_ANY_EVENT);
    ...
    PLIB_CAN_ModuleEventEnable(CAN_ID_2 , 0|CAN_RX_EVENT);
    ...
    PLIB_INT_VectorPrioritySet(INT_ID_0,INT_VECTOR_CAN2, INT_PRIORITY_LEVEL1);
